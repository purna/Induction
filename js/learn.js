(function () {
  ...
  app.startLearn = function (section, level) {
    ...
    const moduleId = app.resolveModuleId(section, app.state.year);
    const path = app.dataPath(moduleId + '-learn.json');
    fetch(path)
      .then(function (res) {
      ...
      .then(function (json) {
        ...
        const saved = app.getLearnProgress(moduleId, level);
        app.state.learnIndex = (saved && saved.lastSlide !== undefined) ? saved.lastSlide : 0;
        app.state.section = moduleId;
  ...
  app.renderLearnMulti = function (exercise) {
    ...
    app.el.exerciseCheckBtn.onclick = function () {
      if (selected.length === 0) {
        app.el.exerciseFeedback.textContent = 'Please select at least one option.';
        app.el.exerciseFeedback.className = 'exercise-feedback wrong';
        return;
      }

      const allCorrect = correctIndices.length === selected.length && correctIndices.every(function (idx) {
